...
class AppStore extends Component {
  state = {tabIdValue: tabsList[0].tabId, value: ''}
  ...
  render() {
    const {tabIdValue, value} = this.state
    const filteredResults = appsList.filter(
      eachItem => eachItem.category === tabIdValue,
    )

    const Results = filteredResults.filter(eachEvent =>
      eachEvent.appName.toLowerCase().includes(value.toLowerCase()),
    )
        ...
        <ul className="tab-list-container">
          {tabsList.map(eachItem => (
            <TabItem
              key={eachItem.tabId}
              eachItem={eachItem}
              onTabApp={this.onTabApp}
              isActive={eachItem.tabId === tabIdValue}
            />
          ))}
        </ul>
